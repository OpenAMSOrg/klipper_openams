...
install_config()
{
    ...
    # oams_macros.cfg -> oams_macros.cfg
...
# Add OpenAMS config include just before [printer]
add_printer_includes()
{
    ...
    # Only add [include oams.cfg] if not already present
    if grep -q '\[include[[:space:]]*oams\.cfg\]' "$printer_cfg"; then
        echo "oams.cfg already included [SKIPPED]"
        return
    fi

    # Insert [include oams.cfg] just before the [printer] section
	awk '/^\[printer\]/{print "[include oams.cfg]"; print ""} {print}' "$printer_cfg" > /tmp/printer_tmp.cfg && mv /tmp/printer_tmp.cfg "$printer_cfg"
    echo "[OK]"
}

# Remove OpenAMS config includes from printer.cfg
remove_printer_includes()
{
    ...
    sed -i '/^\[include oams\.cfg\]/{N;d;}' "$printer_cfg"
    echo "[OK]"
}
...
# Uninstall OpenAMS components from Klipper and Moonraker, and clean config files
uninstall()
{
